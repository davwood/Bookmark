...
feature "User signs up" do
  ...
  scenario "with a password that doesn't match" do
    lambda {sign_up('dave@dave.com','pass','wrong')}.should change(User, :count).by(0)
...
feature "User signs out" do
    ...
    before(:each) do
      User.create(:email => "dave@test.com",
        :password => 'dave',
        :password_confirmation => 'dave')
    end

    scenario 'while being signed in' do
      sign_in('dave@test.com', 'dave')
      click_button "Sign out"
      expect(page).to have_content("Goodbye")
      expect(page).not_to have_content("Welcome, dave@test.com")
    end
